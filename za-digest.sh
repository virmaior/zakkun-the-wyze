...
#		3) filter='[0:v]'"$scaler4"'[z1];[1:v]'"$scaler4"'[z2];[2:v]'"$scaler4"'[z3];[z1][z2][z3]xstack=inputs=3:layout=0_0|0_h0|w0_0|w0_h0:fill=blue[v]';;
	...
	then 
		...
		then
			...
			echo �	ffmpeg -hide_banner -loglevel "$elevel" $( for mkey mval in "${(@kv)ffmap}"; do  printf "%s" "$mval ";  done)  -filter_complex "$filter" -map "[v]" -pix_fmt yuv420p -b:v "$brate" -vsync 2 -c:v "$accel" -an -s hd1080 -r 15 -video_track_timescale 30k  "$outfile.mp4"
