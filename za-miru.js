...
window.miru_tool =
{
    ...
    init: function () {
        this.minutes = document.querySelectorAll(".zminute_DIV");
        this.ranges = document.getElementById("ranges");
        this.run_string = document.getElementById("run_string");
        this.ztd = document.querySelector(".za_top_DIV");
        this.zas = document.querySelectorAll(".za_DIV")

        this.date = this.ztd.getAttribute('day');
        this.cam = this.ztd.getAttribute('cam');
        this.hour = this.ztd.getAttribute('hour');

        let curcam = this.cam;
        if (curcam > 6) { curcam--; }
        document.documentElement.style.setProperty('--cur-cam', curcam);
        this.ztd.classList.add("cam_" + this.cam);
    ...
    generateBar: function () {
        ...
        this.minutes.forEach(el => { el.classList.add('za_nothing'); });
        this.ic.highlight();
    },
    findEarlier(minute) {
...
function generate_ranges() {
    ...
    mt.minutes.forEach((el) => {
        ...
        if (hasStart && hasEnd) {
            ...
            if (!first_minute) {
                ...
                mt.make_range(ranges, minute, minute, rangeName, mt.cam);
            }
            ...
            mt.make_range(ranges, first_minute, minute, rangeName, mt.cam);
            first_minute = false;
        }
    ...
    if (first_minute) {
        ...
        rangeName = getRangeName(first_minute);
        mt.make_range(ranges, first_minute, 59, rangeName, mt.cam);
    }


    if (ranges.length == 0) {
        alert("no ranges marked for camera #" + mt.cam + "\n Stopping Range Generation");
        return false;
    }
    ...
    ranges.forEach(
        (item, index) => {
            ...
            if (!last_start) { last_start = current_start; last_end = current_end; lastName = range_name; }
            else {
                if (current_start == (last_end + 1)) {
                    last_end = current_end;
                } else {
                    mt.make_range(clean_ranges, last_start, last_end, lastName, mt.cam);
                    last_start = current_start; last_end = current_end; lastName = range_name;
                }
            }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let max_min = 0;
    mt.minutes.forEach(zm => {
        const minute = zm.getAttribute('minute');
        const mcount = zm.querySelectorAll('img').length;
        if (mcount > max_min) { max_min = mcount; }
